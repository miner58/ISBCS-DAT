...
from src.training.hyperparameter_tuning.tune_train.base_train_func import test_func
from src.training.hyperparameter_tuning.tune_train.groupDRO_train_func import test_func as groupDRO_test_func
...
def main(experiment_path_list, test_data_default_path, data_config_base_dir, data_type, train_func, test_func):
    for experiment_path in experiment_path_list:
        ...
        if is_cross_domain_experiment(experiment_path):
            ...
            print("🏠 Inner experiment detected - using individual subject configs")
            if data_type == "wire":
                data_config_path_dict = {"B202": [], "N201": [], "R203": []}
            else:  # wireless
                data_config_path_dict = {"B112": [], "N304": [], "N310": []}
...
def is_cross_domain_experiment(experiment_path):
    ...
    3_wireless2wire 또는 4_wire2wireless가 경로에 포함되면 True 반환
    """
...
def make_cross_domain_data_config_dict(data_type, data_config_base_dir):
    ...
    if data_type == "wire":
        ...
        # Wire 파일에서 wireless 데이터를 테스트하는 경우 (4_wire2wireless)
        wireless_config_base_dir = data_config_base_dir.replace("raw3config", "raw5&6config")
        all_config_path = os.path.join(wireless_config_base_dir, "ALL", "dataConfigStimGRL_ALL.json")
        if os.path.exists(all_config_path):
            return {"ALL": [all_config_path]}
        else:
            print(f"Warning: Wireless ALL config file not found at {all_config_path}")
            return {"ALL": []}
    return {"ALL": []}
