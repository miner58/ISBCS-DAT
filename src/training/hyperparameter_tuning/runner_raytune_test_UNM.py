...
def make_cross_domain_data_config_dict(data_type, data_config_base_dir):
    ...
    if data_type == "UNM":
        ...
        # UI 파일에서 UNM 데이터를 테스트하는 경우 (3_UI2UNM)
        unm_config_base_dir = data_config_base_dir.replace("UIconfig", "UNMconfig")
        all_config_path = os.path.join(unm_config_base_dir, "ALL", "dataConfigStimGRL_ALL.json")
        if os.path.exists(all_config_path):
            return {"ALL": [all_config_path]}
        else:
            print(f"Warning: UNM ALL config file not found at {all_config_path}")
            return {"ALL": []}
    return {"ALL": []}
...
if __name__ == "__main__":
    ...
    experiment_path_list = [
        "/home/jsw/Fairness/tmp/Fairness_for_generalization/results_trans/results_trans/results_public_20/3_UI2UNM/ray_results_allMouse_UI_finetune_ReduceLROnPlateau_batch16",
        "/home/jsw/Fairness/tmp/Fairness_for_generalization/results_trans/results_trans/results_public_sub_60/3_UI2UNM/ray_results_allMouse_UI_finetune_ReduceLROnPlateau_LNL_batch16"
    ]
