...
class DMMRFineTuningModule(DMMRBaseLightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # DataModule 출력: (source_data, correspondence_data, subject_ids, source_labels)
        source_data, _, _, source_labels = batch
        ...
        print(f"🔍 FineTune batch shapes: source_data={source_data.shape}, source_labels={source_labels.shape}")
        print(f"🔍 Subject count: {subject_num}, batch_size: {batch_size}")
        
        # 🎯 핵심: subject_num 축 기준으로 피험자별 순환 처리 (원본 DMMR Fine-tuning 방식)
        for subject_idx in range(subject_num):
            try:
                ...
                logits = self.forward(subject_batch_data)
                
                # 분류 손실 계산
                subject_loss = F.cross_entropy(
                    logits, 
                    subject_labels.squeeze() if subject_labels.dim() > 1 else subject_labels
                )
                ...
                actual_labels = subject_labels.squeeze() if subject_labels.dim() > 1 else subject_labels
                ...
                if batch_idx == 0 and subject_idx == 0:
                    print(f"🎯 FineTune Subject {subject_idx}: loss={subject_loss:.4f}")
                    print(f"🔍 Subject shapes: data={subject_batch_data.shape}, labels={subject_labels.shape}")
                
            except Exception as e:
                print(f"⚠️ FineTune Subject {subject_idx} 처리 중 오류: {e}")
                import traceback
                traceback.print_exc()
                continue
        ...
        if total_samples > 0:
            ...
            avg_loss = total_loss / subject_num
        ...
        # 메트릭 업데이트 (전체 배치 기준)
        if total_samples > 0:
            # 모든 피험자 데이터를 flatten해서 메트릭 계산
            all_data = source_data.view(-1, source_data.shape[-2], source_data.shape[-1])  # (batch_size * subject_num, time_steps, features)
            all_labels = source_labels.view(-1)  # (batch_size * subject_num,)
            
            all_preds = torch.argmax(self.forward(all_data), dim=1)
            self.train_metrics.update(all_preds, all_labels)
        
        # 로깅
        self.log_dict({
            'train_loss': avg_loss,
            'train_acc': accuracy,
            'subjects_processed': subject_num,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for subject_idx in range(subject_num):
            try:
                ...
                # Debug info (첫 번째 배치에서만)
                if batch_idx == 0 and subject_idx == 0:
                    print(f"🔍 Val FineTune Subject {subject_idx}: loss={subject_loss:.4f}")
                    print(f"🔍 Val Subject shapes: data={subject_batch_data.shape}, labels={subject_labels.shape}")
                
            except Exception as e:
                print(f"⚠️ Val FineTune Subject {subject_idx} 처리 중 오류: {e}")
                import traceback
                traceback.print_exc()
                continue
        ...
        if total_samples > 0:
            ...
            all_preds = torch.argmax(self.forward(all_data), dim=1)
            self.val_metrics.update(all_preds, all_labels)
        
        # 로깅
        self.log_dict({
            'val_loss': avg_loss,
            'val_acc': accuracy,
            'val_subjects_processed': subject_num,
    ...
    def test_step(self, batch, batch_idx):
        ...
        # 🎯 핵심: subject_num 축 기준으로 피험자별 순환 처리 (test 전용)
        for subject_idx in range(subject_num):
            try:
                # 현재 피험자의 데이터 추출 (축 기준 슬라이싱)
                subject_batch_data = source_data[:, subject_idx, :, :]  # (batch_size, time_steps, features)
                ...
                # Debug info (첫 번째 배치에서만)
                if batch_idx == 0 and subject_idx == 0:
                    print(f"🧪 Test FineTune Subject {subject_idx}: loss={subject_loss:.4f}")
                    print(f"🔍 Test Subject shapes: data={subject_batch_data.shape}, labels={subject_labels.shape}")
                
            except Exception as e:
                print(f"⚠️ Test FineTune Subject {subject_idx} 처리 중 오류: {e}")
                import traceback
                traceback.print_exc()
                continue
        ...
        self.log_dict({
            'test_loss': avg_loss,
            'test_acc': accuracy,
            'test_subjects_processed': subject_num,
