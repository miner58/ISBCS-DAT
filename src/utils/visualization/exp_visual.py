...
class MultiExperimentConfig:
    ...
    def __post_init__(self):
        ...
        if self.save_formats is None:
            self.save_formats = ['png', 'emf', 'svg', 'eps']  # 기본 저장 형식
...
class MultiExperimentVisualizer:
    ...
    def create_multi_experiment_comparison_plot(self, 
        ...
        base_filename = f"multi_exp_{experiment_folder}"
        ...
        for fmt in self.config.save_formats:
            ...
            try:
                if fmt.lower() == 'emf':
                    plt.savefig(save_path, format='emf', bbox_inches='tight', dpi=300)
                    print(f"  💾 EMF 저장 완료 (PowerPoint 편집용): {save_path}")
                elif fmt.lower() == 'svg':
                    plt.savefig(save_path, format='svg', bbox_inches='tight')
                    print(f"  💾 SVG 저장 완료 (벡터 편집용): {save_path}")
                elif fmt.lower() == 'pdf':
                    plt.savefig(save_path, format='pdf', bbox_inches='tight')
                    print(f"  💾 PDF 저장 완료: {save_path}")
                elif fmt.lower() == 'eps':
                    plt.savefig(save_path, format='eps', bbox_inches='tight')
                    print(f"  💾 EPS 저장 완료: {save_path}")
                else:
                    plt.savefig(save_path, format=fmt, bbox_inches='tight', dpi=300)
                    print(f"  💾 {fmt.upper()} 저장 완료: {save_path}")
                    
            except Exception as e:
                print(f"  ❌ {fmt.upper()} 저장 실패: {e}")
    ...
    def _create_multi_table_data(self, means: List[np.ndarray], stds: List[np.ndarray], 
        ...
        """다중 실험 테이블 데이터를 생성합니다. (experiments를 열로 배치)"""
        # 헤더: 첫 번째 행은 실험 라벨들
        table_data = [[""] + experiment_labels]
        csv_data = []
        
        # 각 메트릭(행)에 대해 데이터 생성
        for mi, metric_label in enumerate(self.config.metric_labels):
            row_data = [metric_label]  # 첫 번째 열은 메트릭 라벨
            csv_row = {'Metric': metric_label}
            
            # 각 실험(열)에 대해 mean ± std 추가
            for i, exp_label in enumerate(experiment_labels):
                mean_val = means[i][mi]
                std_val = stds[i][mi]
                formatted_val = f"{mean_val:.3f} ± {std_val:.3f}"
                row_data.append(formatted_val)
                csv_row[exp_label] = formatted_val
